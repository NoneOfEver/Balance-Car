 ...
 * @param color 颜色
 */
void OLED_PrintASCIIChar(uint8_t x, uint8_t y, uint8_t ch, const ASCIIFont *font, OLED_ColorMode color)
{
 ...
 * @param color 颜色
 */
void OLED_PrintNum(uint8_t x, uint8_t y, uint8_t *str, const ASCIIFont *font, OLED_ColorMode color)
{
  uint8_t x0 = x;
  while (*str)
  {
 ...
 * 2. 使用波特律动LED取模工具生成字模(https://led.baud-dance.com)
 */
void OLED_PrintString(uint8_t x, uint8_t y, uint8_t *str, const Font *font, OLED_ColorMode color)
{
  uint16_t i = 0;                                       // 字符串索引
...
/**
  ...
  * @retval 无
  */
void OLED_ShowSignedNum(uint8_t Column,uint8_t Line, int32_t Number, uint8_t Length)
{
	uint8_t i;
	uint32_t Number1;
	if (Number >= 0)
	{
		OLED_PrintASCIIChar(Column,Line, '+',&afont12x6,OLED_COLOR_NORMAL);
		Number1 = Number;
	}
	else
	{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_PrintASCIIChar(Column + i + 1,Line, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0',&afont12x6,OLED_COLOR_NORMAL);
	}
}
// 将浮点数转换为字符串
void floatToStr(float num, char *str, int decimalPlaces) {
    sprintf(str, "%.*f", decimalPlaces, num);
}
/**
 * @brief 绘制带说明文字的ASCII字符串
 * @param x 起始点横坐标
 * @param y 起始点纵坐标
 * @param label 说明文字
 * @param value 数据字符串
 ...
 * @param color 颜色
 */
void OLED_PrintLabeledASCIIString(uint8_t x, uint8_t y, char *label, char *value, const ASCIIFont *font, OLED_ColorMode color)
{
    uint8_t x0 = x;

    // 绘制说明文字
    OLED_PrintASCIIString(x0, y, label, font, color);
    x0 += strlen(label) * font->w;  // 计算下一个字符的横坐标

    // 绘制数据字符串
    OLED_PrintASCIIString(x0, y, value, font, color);
}
